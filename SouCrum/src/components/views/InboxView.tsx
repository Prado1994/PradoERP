...
export function InboxView({ store }: { store: CrmStore }) {
    ...
    >
      ...
      <div style={{ width: 340, flexShrink: 0, borderRight: '1px solid var(--border)', overflowY: 'auto' }}>
        {filtered.map((em) => {
          ...
          const active = em.id === store.selectedEmailId
          const weight = em.unread ? 700 : 500
          return (
            <div
              key={em.id}
              onClick={() => store.selectEmail(em.id)}
              style={{
                display: 'flex',
                gap: 12,
                padding: '14px 18px',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {selected && (
          <>
            <div style={{ padding: '22px 26px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14 }}>
              {(() => {
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-1)' }}>{selected.subject}</div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '22px 26px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {selected.thread.map((msg, i) => (
                <div key={i} style={{ maxWidth: '72%', alignSelf: msg.fromMe ? 'flex-end' : 'flex-start' }}>
                  <div
                    style={{
            ...
            <div style={{ padding: '18px 26px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, alignItems: 'flex-end' }}>
              <textarea
